...
for perm in it.product(range(1, 101), repeat=len(ingredients1)):
    if sum(perm) == 100:
        ...
        for ing, permn in zip(ingredients1, range(len(ingredients1))):
            ...
            cal += ingredients[ing]["calories"] * perm[permn]
        if cap > 0 and dur > 0 and fla > 0 and tex > 0 and cal == 500:
            score.append(int(cap * dur * fla * tex))

print(max(score))  #!11171160
